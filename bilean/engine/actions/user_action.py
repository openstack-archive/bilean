...
class UserAction(base.Action):
    ...
    ACTIONS = (
        USER_CREATE_RESOURCE, USER_DELETE_RESOURCE, USER_SETTLE_ACCOUNT,
    ) = (
        'USER_CREATE_RESOURCE', 'USER_DELETE_RESOURCE', 'USER_SETTLE_ACCOUNT',
    )
    ...
    def __init__(self, target, action, context, **kwargs):
        ...
        try:
            self.user = user_mod.User.load(context, user_id=target)
        except Exception:
            self.user = None
    ...
    def do_settle_account(self):
        if self.user is None:
            LOG.error(_LE('The user(%s) to settle account not found'),
                      self.target)
            return self.RES_ERROR, _('User not found')
        try:
            user.settle_account(self.context, task=self.inputs.get('task'))
        except Exception as ex:
            LOG.error(_LE("Faied to execute action(%(action_id)s), error: "
                          "%(error_msg)s"), {"action_id": self.id,
                                             "error_msg": six.text_type(ex)})
